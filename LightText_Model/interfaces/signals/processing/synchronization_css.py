...
class sync_css:
    ...
    def __init__(self, sampling_rate, symbol_duration, preamble, frame_size, bit_outlet=None, plot=True, bottleneck_deadline=2):
        ...
        self.circular_buffer = FixedLengthQueue(max_size=len(self.preamble))  # Use CircularBuffer for preamble detection
    ...
    def detect_preamble(self):
        ...
        print("cursor target :", self.cursor)
        for i in range(0, len(signal)):
            ...
            if sample != 0 and (sample == 1 or sample == -1):
                ...
                self.circular_buffer.enqueue([i, bit])
                
                # also add code to remove any other sample in half a symbol duration radius
                print("current prepreamble buffer (added): ", self.circular_buffer)
            
            if self.circular_buffer.size() == len(self.preamble):
                # check if the buffer matches the preamble
                circular_buffer_bits = [samp[1] for samp in self.circular_buffer]
                ...
                if circular_buffer_bits == self.preamble:
                    ...
                    distances = [(self.circular_buffer[j + 1][0] - self.circular_buffer[j][0]) for j in range(self.circular_buffer.size() - 1)]
    ...
    def plot_signal(self):
        ...
        cursor_time = self.cursor / self.sampling_rate  # Calculate time for cursor position
...
class FixedLengthQueue:
    ...
    def __str__(self):
        ...
        return f"Queue({', '.join(map(str, self.queue))})"
